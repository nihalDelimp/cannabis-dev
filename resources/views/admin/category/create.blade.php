    ...
    <form method="post" action="{{ route('category.store', app()->getLocale()) }}" enctype="multipart/form-data">
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" name="title" value="{{ old('title') }}" class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="{{langMessage('Category Title')}}" />
          @error('title')
        ...
        <div class="form-group">
          ...
          <textarea name="description" rows="8" cols="80" class="form-control textarea border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="@lang('enter description')"></textarea>
          @error('description')
              <span class="text-danger" role="alert">
                  <strong>{{langMessage($message)}}</strong>
              </span>
