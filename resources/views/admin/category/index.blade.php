    ...
    <div class="box-body">
      ...
      <div class="box box-danger">
        ...
        <div class="box-body">
          ...
          <input class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" name="name" type="text" id="name" placeholder="{{langMessage('Title')}}">
          </div>
          </div>
...
<script>
    $(function(){
      var table = $('#leads').DataTable({
          ...
          "order": [[ 5, "desc" ]],
          "ajax":{
                   url: "{{ route('getCategory',app()->getLocale()) }}",
                   dataType: "json",
                   type: "POST",
                   data:function(data) {
                    data.name = $('#name').val();
