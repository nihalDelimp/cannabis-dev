...
@section('content')
  ...
  <section class="content">
    ...
    @endif
    <form method="post" action="{{ route('news.store', app()->getLocale()) }}" enctype="multipart/form-data">
      @csrf
      <div class="col-md-6">
        <div class="form-group">
          <label for="petrol_saved">{{langMessage('News Title')}}<i class="fa fa-star text-red" aria-hidden="true"></i></label>
          <input type="text" name="title" value="{{ old('title') }}" class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="{{langMessage('News Title')}}" />
          @error('title')
        ...
        <div class="form-group">
          ...
          <input type="text" name="sub_title" value="{{ old('sub_title') }}" class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="{{langMessage('News Sub Title')}}" />
          @error('sub_title')
              <span class="text-danger" role="alert">
                  <strong>{{langMessage($message)}}</strong>
              </span>
        ...
        <div class="form-group">
          ...
          <textarea name="content" rows="8" cols="80" class="form-control textarea border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="@lang('enter your content')">{{langMessage('enter your content')}}</textarea>
          @error('content')
              <span class="text-danger" role="alert">
                  <strong>{{langMessage($message)}}</strong>
              </span>
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          ...
          <select class="form-control select2" multiple="multiple" name="tags_id[]" data-placeholder="{{langMessage('Select')}}">
            @if(!$tags->isEmpty())
              @foreach($tags as $tag)
                <option value="{{$tag->id}}">{{langMessage($tag->title)}}</option>
              @endforeach
            @else
                <option value="">{{langMessage('Empty')}}</option>
            @endif
