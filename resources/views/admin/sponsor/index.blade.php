    ...
    <div class="box-body">
      ...
      <div class="box box-danger">
        ...
        <div class="box-body">
          ...
          <input class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" name="name" type="text" id="name" placeholder="{{langMessage('Sponser Name')}}">
          </div>
          </div>
      ...
      <table id="leads" class="table table-bordered table-hover">
        ...
        <tr>
          ...
          <th>{{langMessage('Sponser Name')}}</th>
          <th>{{langMessage('Status')}}</th>
          <th>{{langMessage('Image')}}</th>
...
<script>
    $(function(){
      var table = $('#leads').DataTable({
          ...
          "order": [[ 0, "desc" ]],
          "ajax":{
                   url: "{{ route('getSponsor',app()->getLocale()) }}",
                   dataType: "json",
                   type: "POST",
                   data:function(data) {
                    data.name = $('#name').val();
