...
@section('content')
  ...
  <section class="content">
    ...
    @endif
    <form method="post" action="{{ route('tag.update', ['tag'=>$tag->id,'locale'=>app()->getLocale()]) }}" enctype="multipart/form-data">
      @csrf
      @method('PATCH')
      <div class="col-md-6">
        <div class="form-group">
          <label for="petrol_saved">{{langMessage('Tag Title')}}<i class="fa fa-star text-red" aria-hidden="true"></i></label>
          <input type="text" name="title" value="{{ $tag->title }}" class="form-control border-0 rounded-0 primary-text-color py-2 pl-3" placeholder="{{langMessage('Category Title')}}" />
          @error('title')
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="post_type">
            <option value="1" {{$tag->post_type == '1' ? 'selected' : '' }}>{{langMessage('News')}}</option>
            <option value="2" {{$tag->post_type == '2' ? 'selected' : '' }}>{{langMessage('Video')}}</option>
          </select>
          @error('post_type')
              <span class="text-danger" role="alert">
                  <strong>{{langMessage($message)}}</strong>
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="status">
            <option value="1" {{$tag->status == '1' ? 'selected' : '' }}>{{langMessage('Active')}}</option>
            <option value="2" {{$tag->status == '2' ? 'selected' : '' }}>{{langMessage('Inactive')}}</option>
          </select>
          @error('status')
              <span class="text-danger" role="alert">
                  <strong>{{langMessage($message)}}</strong>
